...
const useFolders = () =>
  useQuery({
    ...
    queryFn: async () => {
      const res = await contentAPI.getSectionContents(
        "myfile",
        {
          type: "folder",
          limit: 200,
...
export default function NameAndPathSelector({
  fileName,
  setFileName,
  selectedFolder,
  onFolderSelect,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mt-4 md:mt-6 w-full relative">
      {/* Name */}
      <input
        type="text"
        value={fileName}
