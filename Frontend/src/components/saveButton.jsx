...
import {
  useUploadFile, useUploadFolder, useCreateFolder,
  useCreateMessage, useCreateLink,
...
export default function SaveButton({ type, getData, onSuccess, onError }) {
    ...
    try {
      ...
      switch (type) {
        case "file": {
          ...
          if (formData.shareWith?.length)   fd.append("shareWith",    JSON.stringify(formData.shareWith));
        ...
        case "folder-upload": {
          ...
          fd.append("relativePaths",
            JSON.stringify(formData.files.map((f) => f.webkitRelativePath || f.name)));
          if (formData.name?.trim())        fd.append("name",         formData.name.trim());
          if (formData.description)         fd.append("description",  formData.description);
          if (parentFolderId)               fd.append("parentFolder", parentFolderId);
          if (formData.isCoded)             fd.append("isCoded",      "true");
        ...
        case "create-folder": {
          if (!formData.name?.trim()) throw new Error("Folder name is required.");
  ...
  const isPending = loading ||
    ...
    createFolderMutation.isPending || createMessageMutation.isPending ||
  ...
  return (
    <button
      onClick={handleSave}
      disabled={isPending}
      className="flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-2xl hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
    >
