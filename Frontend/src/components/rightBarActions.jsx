...
import {
  FiBell, FiX, FiTrash2, FiLoader, FiEdit2,
  FiLogOut, FiLayout,
} from "react-icons/fi";
...
export default function RightBarActions() {
  ...
  // Poll for new shared content every 30s
  useEffect(() => {
    if (!user) return;
    const check = async () => {
      try {
  ...
  const handleLogout = () => { logoutMutation.mutate(); setOpenProfile(false); user(null); };
  ...
  // Avatar circle — shared between button and dropdown
